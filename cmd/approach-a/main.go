...
func main() {
	var (
		...
		prefix       = flag.String("prefix", "bench:a:active:", "key prefix")
		...
		otelEndpoint = flag.String("otel-endpoint", "otlp-local", "OTEL exporter endpoint label")
	...
	metrics, err := a.Run(ctx, cfg)
	if err != nil {
		fmt.Fprintf(os.Stderr, "run failed: %v\n", err)
		os.Exit(1)
	}

	fmt.Printf("run_id=%s approach=%s tier=%s ingest_ms=%d cutover_ms=%d cleanup_ms=%d duration_ms=%d errors=%v\n",
		metrics.RunID,
		metrics.Approach,
		metrics.Tier,
		metrics.IngestMS,
